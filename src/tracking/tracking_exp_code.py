...
directory = "OUTPUT/" + str(configs.prefix) + "_tracking_amp_" + str(configs.amp) + "_scale_" + str(configs.scale) + "_T_" + str(configs.T) 
...
A, B, Q, R = generate_parameters(mode)
...
print(A.shape)
print(B.shape)
print(generate_w('Tracking',A,1).shape)
...
sinusoidal_noise = np.sin(np.linspace(0, 10*6 , n_points))
...
gauss_noise_comp = np.multiply(np.expand_dims(white_noise,axis=1),  np.asarray([1.2,0.4,0.75,1.25]))
sin_noise_comp = np.multiply(np.expand_dims(sinusoidal_noise,axis=1),  np.asarray([0.8,1.6,1.25,0.75]))

gauss_noise_comp_norm  = gauss_noise_comp / (_MIX_FACTOR* scale_noise)
sin_noise_comp_norm = sin_noise_comp / _MIX_FACTOR

w = generate_w(mode, A, 1000)
Dataset1 = gauss_noise_comp_norm + sin_noise_comp_norm + w 
...
class subDataset(Dataset):
    ...
    def __getitem__(self, index):
        data = torch.FloatTensor(self.Data[index].reshape(4*_COVR_len))
        label = torch.FloatTensor(self.Label[index].reshape(4*_TARG_len))
        return data, label
...
def get_prediction_mixed(w, tr_epochs,t, forw_covr = None, _forw_length = 0):
    ...
    for i in range(len_step-covr_len+1):  
        cov = torch.FloatTensor(w[i:i+covr_len,:].reshape(4*covr_len))
        cov = cov.to(device)
        rep, pred = reg_net(cov)        
        if i != len_step-covr_len:
            pred_lst.append(pred.reshape(targ_len,4)[0,:].cpu().detach().numpy())
        else:
            for j in range(5):
                pred_lst.append(pred.reshape(targ_len,4)[j,:].cpu().detach().numpy())
                
    pred_lst_np = np.asarray(pred_lst)
    
                
    for i in range(t):
        cov = torch.FloatTensor(pred_lst_np[-_COVR_len:,:].reshape(4*covr_len))
        cov = cov.to(device)
        rep, pred = reg_net(cov) 
        
        pred = pred.reshape(targ_len,4)[0,:].cpu().detach().numpy()
        pred_lst_np = np.concatenate( (pred_lst_np, pred.reshape(1,4)) ,axis=0)
...
# Determining lambda (for baseline)
...
def run_online_control_task(T, A, B, Q, R, mode, P, D, H, F, ini_lambda, plot_curve, num_comp, FT):
    ...
    for t in range(T):        
        ...
        estimated_w_selftune = get_prediction_mixed(Dataset1[:t+T0,:] , EST_W_EPOCHS, pred_zeta_step-5) 
    ...
    if plot_curve is True:
        ...
        plt.figure()
        plot_track(_disc_x,y,r'online','blue')        
        plot_track(_optimal_x,y,'Offline-Optimal','red')
        plot_trajectory(y[0:240],'black')
        plt.grid()
        plt.savefig(directory + '/traj_disc.png')    
    ...
    if plot_curve is True:
        ...
        plt.figure()
        plot_track(fulltrust_x,y,r'fulltrust','green')
        plot_track(lqr_x,y,r'lqr','gray')
        plot_track(_selftune_x,y,r'selftune','blue')        
        plot_track(_optimal_x,y,'Offline-Optimal','red')
        plot_trajectory(y[0:240],'black')
        plt.grid()
        plt.savefig(directory + '/traj_selftune.png')        
    ...
    print("selftune CR is")
    print(selftune_ALG/OPT)
    print("LQR CR is")
    print(lqr_CST/OPT)
    print("fulltrust CR is")
    print(fullt_CST/OPT)
